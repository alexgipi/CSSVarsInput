<script lang="ts">
  ...
  if (!labels) labels = { singular: "CSS Variable", plural: "CSS Variables" };
...
<div class="tags-field">
  <div class="tags-field-content">
    {#if Object.keys(varsObject).length > 0}
      <div class:hide={view !== "tags"} class="tags-field-tags">
...
<style>
  ...
  .multi-value-box {
    --active-color: #6cf0ff;
    &.all {
      :global(svg .left, .right, .top, .bottom) {
        stroke: var(--active-color);
      }
    }

    &.x {
      :global(svg .left, .right) {
        stroke: var(--active-color);
      }
    }

    &.y {
      :global(svg .top, .bottom) {
        stroke: var(--active-color);
      }
    }
  ...
  .multi-value-box {
    --active-color: #6cffd7;
    &.all {
      :global(.top-left, .top-right, .bottom-right, .bottom-left) {
